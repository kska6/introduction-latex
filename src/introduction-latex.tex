...
\usepackage{latex-intro-warning-filter}
...
\texttt{biber} を使うために \verb|$bibtex_use = 2| を設定し、\verb|$out_dir| を \file{pdf/}、\verb|$aux_dir| を \file{latex.out/} に分けています。
この設定では、最終的な PDF は \file{pdf/} に置かれ、中間生成物は \file{latex.out/} にまとまります。
補助ファイルを本文と同じ場所へ散らさず、完成物の PDF も中間生成物に埋もれないので、Git 管理でも見通しがよくなります。

\subsection{outdir を分ける理由}

\file{pdf/} と \file{latex.out/} を分ける理由は、主に次の 3 つです。

\begin{itemize}
\item \file{.aux} や \file{.log} が \file{src/} に散らばらない
  \item 完成した PDF が補助ファイルに埋もれない
  \item Git で差分を見るときにノイズが減る
\end{itemize}

入門用の単一ファイルでは同じディレクトリに出しても構いませんが、レポートや卒論のようにファイル数が増えると、PDF を \file{pdf/}、補助ファイルを \file{latex.out/} に分ける運用の恩恵が大きくなります。
...
\begin{itemize}
  ...
  \item \file{latex.out/}: 補助ファイルを隔離する
  \item \file{pdf/}: 完成した PDF をまとめる
...
PDF を成果物として残したいなら \file{pdf/} をどう扱うかを選びやすく、補助ファイルだけが入る \file{latex.out/} を無視する、という分け方がしやすくなります。
...
参照や目次、参考文献まわりが壊れたときは、まず \file{latex.out/} を消して再ビルドすると直ることがあります。
